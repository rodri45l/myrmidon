using TestMyrmidonAPI.MockClasses;
...
public class UserTest
{
    ...
    public UserTest()
    {
        ...
        signInManagerMock.Setup(sim =>
                sim.PasswordSignInAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool>(), It.IsAny<bool>()))
            .ReturnsAsync((string username, string password, bool lockoutOnFailure, bool succeeded) =>
            {
                if (string.IsNullOrEmpty(password))
                    return SignInResult.Failed;
                if (password == "LockedOut")
                    return SignInResult.LockedOut;
                if (password == "NotAllowed")
                    return SignInResult.NotAllowed;
                if (password == "Two") return SignInResult.TwoFactorRequired;
...
// Create a mock ISessionTokenRepository object and pass the mock IMemoryCache object to its constructor
        ...
        _authenticationService = new UserAuthenticationService(_userManager, _mapper, _myrmidonContext,
